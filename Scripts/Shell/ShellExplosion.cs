    ...
    public class ShellExplosion : MonoBehaviour
    {
        ...
        private void OnTriggerEnter (Collider other)
        {
            ...
            for (int i = 0; i < colliders.Length; i++)
            {
                ...
                {
                    // If it's not a tank, try to damage aliens
                    AlienHealth alienHealth = targetRigidbody.GetComponent<AlienHealth>();
                    if (alienHealth != null)
                    {
                        // Deal damage to the alien.
                        alienHealth.TakeDamage(damage);
                        Debug.Log($"[ShellExplosion] 炮弹对外星人 {targetRigidbody.name} 造成 {damage} 点伤害");
                    }
                }
            }
