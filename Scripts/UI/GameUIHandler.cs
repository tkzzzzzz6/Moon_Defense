...
namespace Tanks.Complete
{
    ...
    public class GameUIHandler : MonoBehaviour
    {
        ...
        void StartGame()
        {
            Debug.Log("[GameUIHandler] 开始游戏按钮被点击");
            
            // Check if GameManager is assigned
            if (m_GameManager == null)
            {
                Debug.LogError("[GameUIHandler] GameManager未分配！无法开始游戏");
                return;
            }
            ...
            if (m_SlotUsed < 2)
            {
                Debug.LogWarning("[GameUIHandler] 需要至少2个坦克才能开始游戏");
                return;
            }

            Debug.Log($"[GameUIHandler] 准备开始游戏，使用坦克数量: {m_SlotUsed}");
            ...
            foreach (var slot in m_PlayerSlots)
            {
                if (!slot.IsOpen)
                {
                    Debug.Log($"[GameUIHandler] 添加坦克: 颜色={slot.m_SlotColor}, 是否电脑={slot.IsComputer}, 控制索引={slot.PlayerControlling}");
            ...
            Debug.Log($"[GameUIHandler] 向GameManager传递 {playerData.Count} 个坦克数据");
            ...
            Debug.Log("[GameUIHandler] 游戏启动流程完成");
        }
